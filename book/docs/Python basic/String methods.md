...
| _join(*iterable*)_ | Данный метод создает строку из всех элементов введенной последовательности, разделенных сепаратором (самой строкой) |  |
| ljust(*length[, character]*) | Создает отступ справа от строки |  |
...
| _partition(*value*)_ | Метод ищет в строке введенное значение. Если значение присутсвует в строке, то возвращает кортеж из 3 элементов. Первый элемент - срез строки до найденного значения, второй - само значение, третий - остальная часть строки |  |
| _replace(*oldvalue, newvalue[, count]*)_ | Метод ищет в строке введенное значение. Если значение присутсвует в строке, то возвращает кортеж из 3 элементов. Первый элемент - срез строки до найденного значения, второй - само значение, третий - остальная часть строки |  |

| rjust(*length[, character]*) | Создает отступ слева от строки |  |
