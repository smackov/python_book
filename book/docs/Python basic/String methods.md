...
| lstrip(*characters*) | Возвращает строку с удаленными ведущими символами, еслил они входят в множество `characters` (по умолчанию множетсво содержит только знак пробела) |  |
| __partition(*value*)__ | Метод ищет первое совпадение в строке. Если значение присутсвует в строке, то возвращает кортеж из 3 элементов. Первый элемент - срез строки до найденного значения, второй - само значение, третий - остальная часть строки |  |
...
| __rpartition(*value*)__ | Метод ищет последние совпадение в строке. Если значение присутсвует в строке, то возвращает кортеж из 3 элементов. Первый элемент - срез строки до найденного значения, второй - само значение, третий - остальная часть строки |  |
